...
/**
 * Run a shell command that requires shell features (pipes, redirects, etc.).
 * Uses execSync with shell enabled. Returns stdout on success, descriptive error on failure.
 */
export function shell(cmdStr: string, opts: { timeout?: number } = {}): string {
  try {
    ...
    return `[command failed: ${cmdStr} (exit ${e.status ?? "?"})]`;
  }
}
