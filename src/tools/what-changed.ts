...
export function registerWhatChanged(server: McpServer): void {
  server.tool(
    ...
    async ({ since }) => {
      ...
      const diffFiles = shell(`git diff ${ref} --name-only 2>/dev/null || git diff HEAD~3 --name-only`);
      const log = shell(`git log ${ref}..HEAD --oneline 2>/dev/null || git log -5 --oneline`);
