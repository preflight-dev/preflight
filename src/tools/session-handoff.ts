...
/** Run an external (non-git) command safely, returning stdout or empty string on failure */
function runExternal(cmd: string, args: string[]): string {
  try {
  ...
  server.tool(
    ...
    async ({ direction }) => {
      ...
      if (direction === "incoming") {
        ...
        if (hasCommand("gh")) {
          const openPRs = runExternal("gh", ["pr", "list", "--state", "open", "--json", "number,title,headRefName"]) || "[]";
          if (openPRs && openPRs !== "[]") {
