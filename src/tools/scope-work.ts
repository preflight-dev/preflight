...
import { run, getBranch, getRecentCommits, getStatus, shell } from "../lib/git.js";
  ...
  server.tool(
    ...
    async ({ task, branch }) => {
      ...
      if (grepTerms.length > 0) {
        ...
        const allTracked = run(["ls-files"]).split("\n");
        const re = new RegExp(pattern, "i");
        matchedFiles = allTracked.filter(f => re.test(f)).slice(0, 30).join("\n");
      }
