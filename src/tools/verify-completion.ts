...
/** Run a shell command safely, returning last N lines of output */
function shell(cmd: string, opts: { timeout?: number; tailLines?: number } = {}): string {
  const { timeout = 30000, tailLines = 20 } = opts;
  try {
...
export function registerVerifyCompletion(server: McpServer): void {
  server.tool(
    ...
    async ({ task_description, test_scope, skip_tests, skip_build }) => {
      ...
      const tscOutput = shell(`${pm === "npx" ? "npx" : pm} tsc --noEmit 2>&1`, { tailLines: 20 });
      ...
      if (!skip_tests) {
        ...
        const changedFiles = (!changedFilesRaw || changedFilesRaw.startsWith("[")) ? [] : changedFilesRaw.split("\n").filter(Boolean);
        let testCmd = "";
        ...
        if (runner === "playwright") {
          ...
          if (test_scope && test_scope !== "all") {
            testCmd = test_scope.endsWith(".spec.ts") || test_scope.endsWith(".test.ts")
              ? `${runnerCmd} ${test_scope} --reporter=line 2>&1`
              : `${runnerCmd} --grep "${test_scope}" --reporter=line 2>&1`;
          } else {
            // Auto-detect from changed files
            const changedTests = changedFiles.filter(f => /\.(spec|test)\.(ts|tsx|js)$/.test(f)).slice(0, 5);
            if (changedTests.length > 0) {
              testCmd = `${runnerCmd} ${changedTests.join(" ")} --reporter=line 2>&1`;
            }
          }
        } else if (runner === "vitest" || runner === "jest") {
          const runnerCmd = `${pm === "npx" ? "npx" : `${pm} exec`} ${runner}`;
          if (test_scope && test_scope !== "all") {
            testCmd = `${runnerCmd} --run ${test_scope} 2>&1`;
          } else {
            const changedTests = changedFiles.filter(f => /\.(spec|test)\.(ts|tsx|js)$/.test(f)).slice(0, 5);
            if (changedTests.length > 0) {
              testCmd = `${runnerCmd} --run ${changedTests.join(" ")} 2>&1`;
            }
          }
        } else if (test_scope) {
        ...
        if (testCmd) {
          const testResult = shell(testCmd, { timeout: 120000, tailLines: 20 });
      ...
      if (!skip_build && hasBuildScript()) {
        const buildCheck = shell(`${pm === "npx" ? "npm run" : pm} build 2>&1`, { timeout: 60000, tailLines: 10 });
