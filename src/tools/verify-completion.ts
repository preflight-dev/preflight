...
/** Run a shell command in PROJECT_DIR, returning stdout (or empty string on failure). */
...
export function registerVerifyCompletion(server: McpServer): void {
  server.tool(
    ...
    async ({ task_description, test_scope, skip_tests, skip_build }) => {
      ...
      // 1. Type check
      const tscCmd = `${pm === "npx" ? "npx" : pm} tsc --noEmit 2>&1`;
      const tscOutput = shell(tscCmd, { timeout: 60000 });
      ...
      if (!skip_tests) {
        ...
        if (testCmd) {
          const rawTestResult = shell(testCmd, { timeout: 120000 });
          const testResultLines = rawTestResult.split("\n");
          const testResult = testResultLines.slice(-20).join("\n");
      ...
      if (!skip_build && hasBuildScript()) {
        const rawBuild = shell(`${pm === "npx" ? "npm run" : pm} build 2>&1`, { timeout: 60000 });
        const buildCheck = rawBuild.split("\n").slice(-10).join("\n");
