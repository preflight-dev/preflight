...
/** Run a shell command (non-git) and return stdout, or empty string on failure */
function execShell(cmd: string, args: string[], opts: { timeout?: number } = {}): string {
  try {
...
export function registerVerifyCompletion(server: McpServer): void {
  server.tool(
    ...
    async ({ task_description, test_scope, skip_tests, skip_build }) => {
      ...
      const tscFull = execShell(tscCmd, tscArgs, { timeout: 60000 });
      ...
      if (!skip_tests) {
        ...
        // Build test command as [cmd, ...args] for execShell
        ...
        if (runner === "playwright") {
          ...
          if (test_scope && test_scope !== "all") {
            testArgs = { cmd: baseCmd, args: [...baseArgs, test_scope] };
          } else {
            const changedTests = changedFiles.filter(f => /\.(spec|test)\.(ts|tsx|js)$/.test(f)).slice(0, 5);
            if (changedTests.length > 0) {
              testArgs = { cmd: baseCmd, args: [...baseArgs, ...changedTests] };
            }
          }
        } else if (test_scope) {
        ...
        if (testArgs) {
          const testFull = execShell(testArgs.cmd, testArgs.args, { timeout: 120000 });
          const testResult = testFull.split("\n").slice(-20).join("\n");
      ...
      if (!skip_build && hasBuildScript()) {
        ...
        const buildFull = execShell(buildCmd, buildArgs, { timeout: 60000 });
