...
/** Sanitize user input for safe use in regex/matching */
...
function findAreaFiles(area: string): string {
  ...
  if (area.includes("/")) {
    return run(["ls-files", "--", `${safeArea}*`]).split("\n").filter(Boolean).slice(0, 20).join("\n");
  }

  // Search for area keyword in git-tracked file paths
  const allFiles = run(["ls-files"]).split("\n").filter(Boolean);
  const matched = allFiles.filter(f => f.toLowerCase().includes(safeArea.toLowerCase())).slice(0, 20);
...
function findRelatedTests(area: string): string {
  ...
  const safeArea = sanitize(area.split(/\s+/)[0]).toLowerCase();
