...
function findAreaFiles(area: string): string {
  ...
  if (area.includes("/")) {
    return shell(`git ls-files -- '${safeArea}*' | head -20`);
  }

  // Search for area keyword in git-tracked file paths
  const files = shell(`git ls-files | grep -i '${safeArea}' | head -20`);
...
function findRelatedTests(area: string): string {
  if (!area) return shell("git ls-files | grep -E '\\.(spec|test)\\.(ts|tsx|js|jsx)$' | head -10");

  const safeArea = shellEscape(area.split(/\s+/)[0]);
  const tests = shell(`git ls-files | grep -E '\\.(spec|test)\\.(ts|tsx|js|jsx)$' | grep -i '${safeArea}' | head -10`);
  return tests || shell("git ls-files | grep -E '\\.(spec|test)\\.(ts|tsx|js|jsx)$' | head -10");
}
