  ...
  server.tool(
    ...
    "Export timeline data as a structured markdown report. Generates summaries, daily breakdowns, activity stats, and weekly trends. Optionally saves to a file.",
    {
      ...
      type: z
        ...
        .default("all"),
      format: z
        .enum(["summary", "detailed", "weekly", "json"])
        .default("detailed")
        .describe(
          "Report format: summary (stats only), detailed (full daily breakdown), weekly (week-level trends), json (machine-readable)",
        ),
      save_to: z
        .string()
        .optional()
    ...
    async (params) => {
      ...
      // JSON format returns structured data instead of markdown
      if (params.format === "json") {
        const jsonReport = {
          generated: now,
          project: proj,
          period: { from: stats.firstEvent.slice(0, 10), to: stats.lastEvent.slice(0, 10) },
          stats: {
            totalEvents: stats.totalEvents,
            activeDays: stats.activeDays,
            avgEventsPerDay: +(stats.totalEvents / Math.max(stats.activeDays, 1)).toFixed(1),
            byType: stats.byType,
            byDay: stats.byDay,
          },
          events: events.map((e: any) => ({
            ...
            type: e.type,
            content: e.content || e.summary || null,
            ...(e.commit_hash ? { commitHash: e.commit_hash } : {}),
        ...
        if (params.save_to) {
          ...
          const filePath = resolve(projectRoot, params.save_to);
          if (!filePath.startsWith(resolve(projectRoot))) {
            return {
              content: [
                {
                  type: "text" as const,
                  text: `Error: save_to path "${params.save_to}" resolves outside the project root. Use a relative path within your project.`,
                },
              ],
            };
          }
          ...
          return {
            content: [
              {
                type: "text" as const,
                text: `JSON report saved to \`${filePath}\` (${output.length} chars, ${stats.totalEvents} events).`,
              },
            ],
          };
        }
      ...
      if (params.save_to) {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: `Report saved to \`${filePath}\` (${output.length} chars, ${stats.totalEvents} events).\n\n${output}`,
            },
          ],
        };
      }
