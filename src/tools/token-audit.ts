...
/** Count lines in a file using Node fs (no shell needed). */
...
/** Get file size in bytes using Node fs. */
...
export function registerTokenAudit(server: McpServer): void {
  server.tool(
    ...
    async ({ session_actions, check_mode }) => {
      ...
      if (claudeMd !== null) {
        const bytes = fileBytes("CLAUDE.md");
        if (bytes > 5120) {
        ...
        if (existsSync(toolLogPath)) {
          try {
            ...
            const raw = stat.size <= MAX_TOOL_LOG_BYTES
              ...
              : shell(`tail -c ${MAX_TOOL_LOG_BYTES} '${toolLogPath.replace(/'/g, "'\\''")}'`);
