...
/** Count lines in a file using pure Node.js (no shell). Returns 0 on error. */
function countLines(filePath: string): number {
  try {
    const full = join(PROJECT_DIR, filePath);
...
/** Get file size in bytes using pure Node.js. Returns 0 on error. */
...
/** Read the tail of a file (last `bytes` bytes). Returns empty string on error. */
function readTail(absPath: string, bytes: number): string {
  try {
