...
function countFileLines(filePath: string): number {
  try {
    const content = readFileSync(join(PROJECT_DIR, filePath), "utf-8");
...
function getFileBytes(filePath: string): number {
  try {
    return statSync(join(PROJECT_DIR, filePath)).size;
  } catch {
    return 0;
  }
}
...
function readFileTail(absPath: string, maxBytes: number): string {
  try {
    const stat = statSync(absPath);
    if (stat.size <= maxBytes) return readFileSync(absPath, "utf-8");
...
export function registerTokenAudit(server: McpServer): void {
  server.tool(
    ...
    async ({ session_actions, check_mode }) => {
      ...
      if (claudeMd !== null) {
        const bytes = getFileBytes("CLAUDE.md");
        if (bytes > 5120) {
        ...
        if (existsSync(toolLogPath)) {
          try {
            ...
            const raw = stat.size <= MAX_TOOL_LOG_BYTES
              ...
              : readFileTail(toolLogPath, MAX_TOOL_LOG_BYTES);
