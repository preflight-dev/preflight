...
export function scorePrompt(text: string): ScoreResult {
  ...
  if (/\b(only|just|single|one|specific|this)\b/i.test(text)) {
    ...
    // Longer prompts suggest more thought, but don't guarantee bounded scope
    scope = 20;
    feedback.push("🎯 Detailed prompt, but consider narrowing scope explicitly (use 'only', 'just', 'this')");
