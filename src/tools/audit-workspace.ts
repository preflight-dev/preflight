    ...
    async () => {
      ...
      if (trackingDocs.length > 0) {
        ...
        if (existsSync(testsDir)) {
          try {
            const walkTests = (dir: string): number => {
              let count = 0;
              for (const entry of readdirSync(dir, { withFileTypes: true })) {
                if (entry.isDirectory()) count += walkTests(join(dir, entry.name));
                else if (/\.(spec|test)\.(ts|js)$/.test(entry.name)) count++;
              }
              return count;
            };
            testFilesCount = walkTests(testsDir);
          } catch { /* ignore */ }
        }
