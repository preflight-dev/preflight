...
export function registerAuditWorkspace(server: McpServer): void {
  server.tool(
    ...
    async () => {
      ...
      const recentFiles = (!recentFilesRaw || recentFilesRaw.startsWith("[")) ? [] : recentFilesRaw.split("\n").filter(Boolean);
      ...
      if (trackingDocs.length > 0) {
        const testFilesCount = (() => {
          try {
            ...
            const entries = readdirSync(testsDir, { recursive: true, withFileTypes: false }) as string[];
            return entries.filter((e: string) => /\.(spec|test)\.ts$/.test(String(e))).length;
          } catch { return 0; }
        })();
